...
- 9c66c10: Fix published package shipping without `dist/`. The build hook was named `prepublish`, which npm (7+) no longer runs before `npm publish` — only before a local `npm install`. Renamed it to `prepublishOnly` and added an explicit build step to the release workflow so `dist/` is always generated before packing.

## 1.0.1

### Patch Changes

- c843d24: Set up the automated release pipeline: Changesets for versioning/changelog, and a GitHub Actions workflow that publishes to npm via Trusted Publishing (OIDC), with no stored npm token.
