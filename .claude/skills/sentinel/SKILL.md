...
- **If the user explicitly asks for everything** ("revisa todos los specs"), honor it — but first correlate by slice name across `deltas/`, so a slice with a sibling `spec.md`/`design.md`/`plan.md` is checked and reported as one slice, not several unrelated hits. Say up front that this dispatches one check per slice (not per file) and will cost proportionally, and offer to narrow first. Don't refuse, just don't default to it.
