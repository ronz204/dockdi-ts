...
Provide asynchronous factory registration (`toAsyncFactory`) and non-blocking dependency resolution (`container.resolveAsync()`) for dependencies requiring asynchronous initialization (e.g. database connection pools, remote config fetching), while protecting the synchronous path (`container.get()`) from promise contamination or unhandled promise rejections.

## Scope

- **Owns**:
  - `toAsyncFactory(factoryFn, tokens)` fluent registration method on `BindingBuilder<T>`.
  - Scoping support (`inSingletonScope()`, `inTransientScope()`, `inResolutionScope()`) on `toAsyncFactory`.
  ...
  - Synchronous execution guard: throwing `AsyncBindingError` when `container.get()` encounters an `asyncFactory` or any dependency path containing an async factory.
...
|---|---|---|---|
| `container.resolveAsync(token)` | `asyncFactory` | Returns `Promise<T>` resolving to dependency | Shared pending `Promise` for `singleton` scope |
...
| `container.get(token)` | `asyncFactory` (or transitive async dep) | Throws `AsyncBindingError` synchronously | Fails fast prior to executing factory |
...
- **Scope parity**: `toAsyncFactory` supports `.inSingletonScope()`, `.inTransientScope()`, and `.inResolutionScope()` with identical lifetime semantics as synchronous factories.
...
- `toAsyncFactory` allows registering asynchronous factories returning `Promise<T>`.
